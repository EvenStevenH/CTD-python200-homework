...
# ML/LLM Question 2
# For converting date string to day-of-week, I'd use deterministic code (datetime library) for simple mapping with no ambiguity.
# For classifying job posting by experience level, I'd use a trained ML model because classification benefits from training on specific examples like labeled text data.
# For predicting customer churn, I'd use a trained ML model on historical behavioral data because predictions requires statistical modeling of complex patterns over time.
# For normalizing city names, I'd use deterministic code with canonical mapping because consistent formatting doesn't require learning; just rule-based transformations.
